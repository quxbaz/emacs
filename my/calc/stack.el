...
(defun my/calc ()
  ...
  (sit-for 0))  ; flush redisplay so trail window hides immediately
...
or top stack entry."
  (interactive)
  (my/calc-replace-expr-dwim (expr replace-expr)
      ((prefix (if (calc-is-hyperbolic) "lg10" "ln")))
    (replace-expr (calc-normalize
                   (if (calc-is-hyperbolic)
...
Works contextually: operates on the entry at point or top of stack."
  (interactive)
  (my/calc-replace-expr-dwim (expr replace-expr) ((prefix "inv") (line? t) (map? -1))
