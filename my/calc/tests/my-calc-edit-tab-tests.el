...
(ert-deftest test-calc-edit-tab-toggle-cycle ()
  ...
  (with-tab-buffer "x+1 = y+2"
    ...
    (my/calc-edit-tab)                    ; → RHS start ('y')
    (should (= (point) 7))
    (my/calc-edit-tab)                    ; → back to LHS (3)
    (should (= (point) 3))
    (my/calc-edit-tab)                    ; → RHS (saved: 7)
    (should (= (point) 7))))
...
(ert-deftest test-calc-edit-tab-insert-then-toggle ()
  ...
  (with-tab-buffer "x+1"
    (goto-char 2)                         ; after 'x'
    (my/calc-edit-tab)                    ; inserts ' = ', cursor after it
    (should (string-match-p "=" (buffer-string)))
    (my/calc-edit-tab)                    ; RHS→LHS: back to pos 2
    (should (= (point) 2))))
